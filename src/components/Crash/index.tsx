...
export default function WebGLStarter() {
	const { GameState, currentNum, time, unityState, myUnityContext, setCurrentTarget, roundId } = React.useContext(Context)
	...
	const planeMultiplierRef = React.useRef<HTMLDivElement>(null);
	...
	React.useEffect(() => {
		if (GameState === "BET") {
			...
			if (planeMultiplierRef.current) {
				planeMultiplierRef.current.style.opacity = '0';
				planeMultiplierRef.current.style.transform = 'translate(0px, 0px)';
			}
			...
			if (planeMultiplierRef.current) {
				planeMultiplierRef.current.style.opacity = '1';
			}
		} else if (GameState === "GAMEEND") {
			setPlaneVisible(false);
			if (planeMultiplierRef.current) {
	...
	React.useEffect(() => {
		if (GameState === "PLAYING" && target > 1) {
			const maxX = 430;
			const maxY = 260;
			const progress = Math.min(1.0, (target - 1.0) / (parseFloat(currentNum) - 1.0 || 1));
			...
			if (planeMultiplierRef.current) {
				planeMultiplierRef.current.style.transition = 'transform 0.1s linear';
				// Position multiplier next to the plane's tail/body
				planeMultiplierRef.current.style.transform = `translate(${x - 45}px, ${y + 20}px)`;
			}
		}
			...
			<div className="space-box" ref={spaceRef} id="space">
				<Unity 
					...
					style={{ 
						...
						height: '100%', 
						zIndex: 1,
						visibility: unityState ? 'visible' : 'hidden'
					}} 
				/>
				<div className="round-id-badge">Round #{roundId || 1}</div>
				<div 
					className={`plane-multiplier ${GameState === "PLAYING" ? 'visible' : ''}`}
					ref={planeMultiplierRef}
				>
					{GameState === "BET" ? '' : target.toFixed(2)}x
				</div>
				<img 
					...
					ref={planeRef}
					style={{ display: 'none' }} /* Hide HTML duplicate plane, WebGL is the primary graphics */
				/>
